...
Testing with *roadacc_month_ppp_jit*

While we indicate "drop = TRUE" in the code above to run jittering, the structure of the *roadacc_month_ppp_jit* remains a list of 99 simulations rather just a single ouput

```{r}
#| eval: false
str(roadacc_month_ppp_jit)
```

As such, to utilise the jittered product, should have selected 1 simulation out of the 99 simulations

```{r}
roadacc_month_ppp_jit_single <- roadacc_month_ppp_jit[[1]]
roadacc_month_jit_owin_ppp <- roadacc_month_ppp_jit_single[provinceowin]
plot(roadacc_month_jit_owin_ppp)
```
:::
...
### 11.4 Calculating Spatio-Temporal NKDE (for both space and time)
...
::: callout-note
## After-assignment reflection on key concepts:

**Kernel Density Estimation (KDE)** is a non-parametric method used to estimate the **intensity** or **probability density** of events (e.g., points in space, data points in statistics) in a continuous space. In the context of spatial analysis, it’s often used to create a smooth surface or map showing where events (like accidents, crimes, or wildlife sightings) are concentrated.

-   **Kernel**: A kernel is a smooth, symmetric function (often a Gaussian or Epanechnikov function) that is placed over each data point. The idea is to "spread" the influence of each point over its neighboring area, instead of assigning it to just one location.

-   **Density**: The density at any location is the sum of the contributions of all the nearby kernels. This results in a smoothed surface that shows where points are more concentrated and where they are more sparse.

KDE is commonly used in geographical information systems (GIS) to visualize the spatial intensity of events (e.g., where road accidents or crimes are most frequent in a city).

### **Key Concepts in KDE**

1.  **Bandwidth**:

    -   The **bandwidth** controls the **spread** of the kernel around each point. It defines how far the influence of each point extends into surrounding space. A **larger bandwidth** produces a smoother, more generalized estimate, while a **smaller bandwidth** produces a more detailed, localized estimate.

        -   **Small bandwidth**: More sensitive to local variations but can result in a noisy or overfitted estimate.

        -   **Large bandwidth**: Produces a smoother estimate but can oversmooth the data, losing important local details.

2.  **Fixed vs. Adaptive Bandwidth**:

    -   **Fixed Bandwidth**: The same bandwidth (distance) is applied uniformly across the study area. Each point has the same influence regardless of how densely or sparsely the points are distributed.

        -   Useful when the data is evenly distributed, but it can cause oversmoothing in sparse areas or undersmoothing in dense areas.

    -   **Adaptive Bandwidth**: The bandwidth varies depending on the **local density of points**. In areas with a high concentration of points, the bandwidth is smaller to capture local variations, while in sparse areas, the bandwidth is larger to smooth out the density estimate.

        -   Useful when data is unevenly distributed. It can avoid oversmoothing in dense areas and undersmoothing in sparse areas, producing a more balanced estimate across the study area.

### **Nearest Neighbor Analysis (NNA)**

**Nearest Neighbor Analysis (NNA)** is a method used to measure the **spatial arrangement** of points by looking at the distances between each point and its nearest neighbor. It’s typically used to determine if points are **clustered, randomly distributed**, or **dispersed**.

### Why need to convert gridded output into a Raster Layer:

### 1. **Integration with Raster-Specific Tools and Packages**:

-   The **`raster` package** in R, and other raster-oriented GIS tools, require data in a raster format. Converting the gridded output into a raster allows you to:

    -   Use additional raster-specific functions for analysis, such as resampling, masking, or calculating statistics over raster layers.

    -   Export your KDE output as a raster file (e.g., GeoTIFF) for use in other GIS applications, like QGIS or ArcGIS, which are optimized for working with raster data.

### 2. **Enhanced Visualization**:

-   Although you can plot a gridded object (like the result of `as.im()`), some high-level visualization libraries like **`tmap`** (used for thematic mapping) expect or work more efficiently with raster objects. For instance, the `tm_raster()` function in **`tmap`** specifically requires raster data to create continuous color maps.

    -   The `raster()` function converts the gridded KDE output to a format that `tmap` can easily interpret, allowing you to use features like color palettes (`viridis`) and layout customization.

### 3. **Consistency Across Spatial Data Types**:

-   In many workflows, you might be working with a combination of spatial data types, including **points**, **polygons**, and **rasters**. Converting the KDE output to a raster allows you to **overlay** it with other raster data layers or perform operations like spatial interpolation, zonal statistics, or combining multiple raster datasets in a consistent manner.

### 4. **Exporting and Sharing**:

-   **Raster layers** are one of the most common formats for spatial data, and exporting your KDE result as a raster makes it easier to share with others who may be using different software (e.g., QGIS, ArcGIS). Many tools outside R expect raster formats for handling continuous surfaces like density estimates.

### 5. **Spatial Alignment and Projection**:

-   When converting to raster, you can ensure the data aligns correctly with other spatial datasets in your analysis. Rasters are often used for geospatial operations that require spatial referencing (e.g., aligning layers with different coordinate reference systems).

### Clark Evans Test

In the **`clarkevans.test()`** function from the **`spatstat`** package, the `alternative` argument specifies the type of hypothesis you are testing regarding the **spatial arrangement** of points in your point pattern. This test is used to determine whether the points are **randomly distributed** (complete spatial randomness, or CSR), **clustered**, or **regularly spaced**.

### Explanation of the `alternative` Argument

The `alternative` argument allows you to specify which type of alternative hypothesis you're testing against the null hypothesis (which is usually CSR). The options you provided are `"clustered"` and `"two.sided"`, and they correspond to different types of alternative hypotheses:

1.  **`alternative = "clustered"`**:

    -   **Hypothesis**:

        -   **Null Hypothesis**: The points are distributed randomly (CSR).

        -   **Alternative Hypothesis**: The points are **more clustered** than expected under CSR.

    -   **Use case**: You use this option if you are specifically interested in testing whether the points exhibit **clustering** (i.e., many points are closer together than they would be under random conditions). The test will only consider this alternative and will look for evidence of clustering.

    -   **Interpretation**: If the test rejects the null hypothesis, it will suggest that the points are clustered. If the null hypothesis is not rejected, the points could be randomly distributed or exhibit some other pattern (but not necessarily regularity).
...
2.  **`alternative = "two.sided"`**:
    ...
    -   **Hypothesis**:
        ...
        -   **Alternative Hypothesis**: The points are either **clustered** or **regularly spaced** (departures in either direction from CSR).

    -   **Use case**: You use this option if you're open to either possibility — clustering or regular spacing — and want the test to detect **any departure from CSR**, whether the points are clustered or regularly spaced.

    -   **Interpretation**: If the test rejects the null hypothesis, it suggests that the point pattern is either **clustered** or **regularly spaced**. The test will not specify which one, and further analysis may be needed to determine the nature of the pattern.

### Difference Between `"clustered"` and `"two.sided"`:

-   **`"clustered"`**: Tests only for **clustering**. The test looks for evidence that points are closer together than expected under random conditions. If you are specifically looking for clustering patterns in your data, use this option.

-   **`"two.sided"`**: Tests for **both clustering and regular spacing**. The test checks for any deviation from randomness, in either direction (points are either too close or too far apart). Use this option if you're interested in detecting **any non-random pattern**, whether it's clustering or regularity.

### When to Use Each Option:

-   **Use `"clustered"`** if:

    -   You have a specific hypothesis that the points should be **clustered** (e.g., incidents like road accidents might be expected to occur in clusters around intersections).

    -   You only care about clustering and don’t need to test for regular spacing.

-   **Use `"two.sided"`** if:

    -   You are unsure about the nature of the point distribution and want to detect **any deviation** from randomness, whether it be clustering or regular spacing (e.g., in environmental data, some species might cluster, while others might exhibit regular spacing due to territorial behavior).

    -   You want to check for **both types of deviation** (clustered or regular) in one test.

### Summary:

-   **`alternative = "clustered"`** tests whether the points are more **clustered** than expected under random conditions.

-   **`alternative = "two.sided"`** tests whether the points are **either clustered or regularly spaced**, detecting any non-random pattern.

Choose `"clustered"` if you're only concerned with clustering, and `"two.sided"` if you want to test for any kind of non-random distribution.

The **G**, **F**, **K**, and **L functions** are key tools in **spatial point pattern analysis**. They help describe and test the spatial distribution of points by analyzing their spatial relationships and comparing them to a random distribution (Complete Spatial Randomness, or CSR). These functions assess different aspects of the distance between points, providing insight into whether points are **clustered**, **dispersed**, or **randomly distributed**.
:::
