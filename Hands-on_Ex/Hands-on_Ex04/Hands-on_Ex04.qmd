...
1.  **`nb2listw(wm_q, glist=ids, style="B", zero.policy=TRUE)`**:
...
2.  **`rswm_ids`**: This stores the resulting **spatial weights matrix** (weights list object) created by `nb2listw()`.
:::
...
### 4.10.2 Spatial lag as a sum of neighbouring values
