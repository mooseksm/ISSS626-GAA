...
```{r}
pacman::p_load(sf,spdep,GWmodel,SpatialML,tmap,rsample,Metrics, tidyverse,spatstat,kableExtra)
```
...
::: callout-note
## In-class note from Prof Kam

-   note that proximity to xx are derived values - note that there are some inconsistencies in the values i.e. some too small

-   **to overcome overlap, need to use st_jitter():**

    -   prof shared that there is either amount or factor - **is better to use amount since its based on units of coordinates**

    -   put amount as 2 - don't use v big figures **but also don't use decimal points as when there's rounding, it might round to 0 then back to square one**

    -   must be careful about the distance set to avoid jittering the point out of the boundary
...
We split the data set into training (65%) and test (35%) data sets by using *initial_split()* of [**rsample**](https://rsample.tidymodels.org/) package. rsample is a [**tidymodels**](https://www.tidymodels.org/) package (collection of tools to do modelling, new framework to build modelling pipeline):
...
-   note that the above is random sampling method

-   65% is train and 35% is test

-   can immediately pull out training and test data set as separate datasets

-   always save the training and test datasets as new rds files
:::
...
-   another way to generate the correlation matrix is using the code chunk below:
...
-   note that only use training data to calibrate the model, will not use test data to calibrate

-   we can use olsrr to generate report - provide a more organised way to look through results:
...
-   can use kable to print out a nice table from tibble data
...
-   plot above is user-friendly, at one glance can see all predictors are not highly correlated
:::
...
-   Note that gw can now handle non-sp data format hence the codes below use train_data and test_data directly and not the sp formats
-   Predictive Modelling with MLR:

```{r}
#| eval: false
bw_adaptive_train <- bw.gwr(resale_price ~ floor_area_sqm + storey_order + remaining_lease_mths + PROX_CBD + PROX_ELDERLYCARE + PROX_HAWKER + PROX_MRT + PROX_PARK+ PROX_MALL + PROX_SUPERMARKET + WITHIN_350M_KINDERGARTEN + WITHIN_350M_CHILDCARE + WITHIN_350M_BUS + WITHIN_1KM_PRISCH,
                data = train_data,
                approach = "CV",
                kernel = "gaussian",
                adaptive = TRUE,
...
```{r}
#| eval: false
gwr_bw_test_nonsp_adaptive <- bw.gwr(resale_price ~ floor_area_sqm + storey_order + remaining_lease_mths + PROX_CBD + PROX_ELDERLYCARE + PROX_HAWKER + PROX_MRT + PROX_PARK+ PROX_MALL + PROX_SUPERMARKET + WITHIN_350M_KINDERGARTEN + WITHIN_350M_CHILDCARE + WITHIN_350M_BUS + WITHIN_1KM_PRISCH,
                data = test_data,
                approach = "CV",
                kernel = "gaussian",
                adaptive = TRUE,
...
```{r}
#| eval: false
gwr_pred_train <- gwr.predict(resale_price ~ floor_area_sqm + storey_order + remaining_lease_mths + PROX_CBD + PROX_ELDERLYCARE + PROX_HAWKER + PROX_MRT + PROX_PARK+ PROX_MALL + PROX_SUPERMARKET + WITHIN_350M_KINDERGARTEN + WITHIN_350M_CHILDCARE + WITHIN_350M_BUS + WITHIN_1KM_PRISCH,
                data = train_data,
...
-   Likely issue is several points with same postal code hence same coordinates - spatial ML can tolerate but gwr unable to handle hence error of "No regression point is fixed"

-   Could have averaged HDB resale price but not good idea as different units at the same postal code could have varying prices

-   Instead, we do a jitter to carry out slight displacement of the points:

    -   note that this [SAS reference](https://blogs.sas.com/content/iml/2011/07/06/to-jitter-or-not-to-jitter-that-is-the-question.html) shared that it's not a good idea as its for aspatial data

    -   BUT jittering in our exercise just change the x, y value of the coordinates and not the attribute values

-   When it comes to test data, question of whether we use bw used to calibrate train data or whether we determine new bw for test data:

    -   if the former, is bw = 40 for the school of thought to have parity

    -   if the latter, bw = 25, as test data is different from training data

    Prof shared that [either method above works]{.underline} but what extent to which results will be sensitive to the bw - for HDB property transaction, Prof didn't find much difference. If data set is small, high chance will have variation but if data set is big and use method such as random forest (for which have a lot of randomisation), should be able to resolve.
:::

## 8.8 Preparing coordinates data (for Predictive Modelling via SpatialML method)
...
-   **note that need to extract coordinates (coords) as dataframe** which will be used to calibrate model - SpatialML unable to use train and test data directly

    -   **note from grf documentation that only can accept dframe** - one for dependent variable and one for independent variable. coords must also be in the sequence x and y.

-   **need to drop geometry column from train_data to make it from sf data frame to normal data frame**, otherwise will keep getting error when running SpatialML
...
-   note that dframe and coords have to be consistent i.e. both derive from training data
:::
...
-   **Query on why bw = 55 is used \>\> Prof shared that same as above steps, will have to calculate bw and use that bw**
-   note in gwRF_adaptive, LGofFit means Local Goodness of Fit
-   note that by default trees = 500 for grf function, can adjust ntree to change no. of trees
:::
...
-   note that above combine test_data with coords_test as next code chunk requires the data in the same table i.e. x.var.name = "X" and y.var.name = "Y", this is different from what was done for train_data under the SpatialML methods as the code required a separate coords file for the coords argument i.e. coords = coords_train
...
-   above gives predicted values from test data
...
-   note that as we are doing geospatial, we are interested in the distribution of the results over space

-   to check if the above means that we will need to visualise the distribution via tmap etc rather than just a scatterplot
:::

::: callout-note
## In-class notes from Prof Kam:

-   important to first determine bandwidth for gwRF then calibrate model - technically the same bandwidth should be used throughout the geographically weighted models to ensure that samples are consisten

-   sample size: one predictor should have \~100 observations to have a good predictive analysis. note that random forest will try to overcome if sample size is slightly smaller. in literature, min. 50 rows of data per variable but ideally 100 observations for each predictor
:::
